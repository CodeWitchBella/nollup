...
function resolveMetaProperty (context, input, output, node, currentpath) {
    if (node.meta_property && node.meta_property.name.startsWith('ROLLUP_')) {
        if (node.meta_property.name.startsWith('ROLLUP_FILE_URL_')) {
            let id = node.meta_property.name.replace('ROLLUP_FILE_URL_', '');
